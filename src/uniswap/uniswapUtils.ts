...
export function getTransferEventSignature(): string {
...
export function getSwapEventSignature(): string {
...
// Provide Liquidity
export function getMintEventSignature(): string {
    const event = 'Mint(address,uint256,uint256)';
    const event_bytes = ethers.utils.toUtf8Bytes(event);
    const sig = ethers.utils.keccak256(event_bytes);
    return(sig)
}

// Withdraw Liquidity
export function getBurnEventSignature(): string {
    const event = 'Burn(address,uint256,uint256,address)';
    const event_bytes = ethers.utils.toUtf8Bytes(event);
    const sig = ethers.utils.keccak256(event_bytes);
    return(sig)
}
